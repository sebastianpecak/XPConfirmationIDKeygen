...
static void __attribute__((unused)) divisor_mul(const TDivisor* src, ui64 mult, TDivisor* dst)
{
    if (mult == 0) {
        dst->u[0] = BAD;
        dst->u[1] = BAD;
...
{
    switch (err) {
    case ERR_TOO_SHORT: return "Installation ID is too short";
    case ERR_TOO_LARGE: return "Installation ID is too large";
    case ERR_INVALID_CHARACTER: return "Installation ID contains an invalid character";
    case ERR_INVALID_CHECK_DIGIT: return "Installation ID has an invalid check digit";
    case ERR_UNKNOWN_VERSION: return "Unknown installation ID version";
    case ERR_UNLUCKY: return "Failed to derive confirmation ID (unlucky ID)";
...
int main(int argc, char** argv)
{
    ...
    if (argc > 1) {
        ...
        for (int i = 1; i < argc; i++) {
            ...
            if (written + arg_len + 1 >= sizeof(installation_id)) {
                fprintf(stderr, "Input too long\n");
                return 1;
            }
            if (i > 1)
                installation_id[written++] = ' ';
            memcpy(installation_id + written, argv[i], arg_len);
            written += arg_len;
        }
        installation_id[written] = '\0';
    } else {
        if (!fgets(installation_id, sizeof(installation_id), stdin)) {
            fprintf(stderr, "No installation ID provided\n");
            return 1;
        }
        installation_id[strcspn(installation_id, "\r\n")] = '\0';
    }
